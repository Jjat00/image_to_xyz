...
import { AppState, PointCloudData, ProcessingConfig } from './types';
...
function App() {
  ...
  const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (e) => {
      ...
      try {
        // Step 1: Generate Depth Map via Gemini
        const depthBase64 = await generateDepthMap(base64Data, file.type);
        ...
        console.error(err);
        setErrorMsg(err.message || "Failed to generate depth map. Please check your API Key and try again.");
  ...
  const processPoints = async (rgb: string, depth: string, currentConfig: ProcessingConfig) => {
      try {
        const data = await generatePointsFromImages(rgb, depth, currentConfig.sampleRate, currentConfig.depthScale);
        setPointData(data);
      ...
      <header className="bg-slate-900/80 backdrop-blur border-b border-slate-700 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 py-4 flex flex-wrap gap-4 justify-between items-center">
            <div className="flex items-center gap-3">
                ...
                <div>
                    <h1 className="text-xl font-bold text-white tracking-tight">DepthGen 3D</h1>
                    <p className="text-xs text-slate-400 hidden sm:block">Powered by Gemini 2.5</p>
                </div>
            </div>
             ...
             `}>
                <input 
                    ...
                    disabled={state === AppState.GENERATING_DEPTH || state === AppState.PROCESSING_POINTS}
                />
        ...
        <div className="lg:col-span-3 flex flex-col gap-6 lg:overflow-y-auto lg:pr-2 custom-scrollbar">
            ...
            {!rgbImage && !errorMsg && state === AppState.IDLE && (
                 <div className="p-6 border-2 border-dashed border-slate-700 rounded-xl text-center space-y-4">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-slate-400 mt-1">Upload a photo to generate a 3D point cloud.</p>
                    </div>
                 </div>
            )}
            ...
            {rgbImage && (
                <div className="bg-slate-800 rounded-lg overflow-hidden border border-slate-700 shadow-md group relative">
                    <div className="absolute top-2 left-2 bg-black/60 backdrop-blur px-2 py-0.5 rounded text-xs font-mono text-white/80 z-10">RGB Source</div>
                    <img src={rgbImage} alt="Source" className="w-full h-auto object-cover" />
                </div>
            )}
            ...
            {depthImage ? (
                <div className="bg-slate-800 rounded-lg overflow-hidden border border-slate-700 shadow-md group relative">
                    <div className="absolute top-2 left-2 bg-black/60 backdrop-blur px-2 py-0.5 rounded text-xs font-mono text-white/80 z-10">Generated Depth Map</div>
                    <img src={depthImage} alt="Depth" className="w-full h-auto object-cover grayscale" />
                </div>
            ) : (
                state === AppState.GENERATING_DEPTH && (
                    <div className="aspect-video bg-slate-800/50 rounded-lg border border-slate-700 flex flex-col items-center justify-center gap-3 animate-pulse">
                         ...
                         <p className="text-sm text-indigo-300 font-medium">Generating depth map with Gemini...</p>
                    </div>
                )
            )}
             ...
             <div className="w-full h-full bg-black/40 rounded-xl border border-slate-800 flex items-center justify-center backdrop-blur-sm">
                {state === AppState.PROCESSING_POINTS ? (
                     <div className="text-center space-y-3">
                         ...
                         <h3 className="text-xl font-bold text-white">Constructing Point Cloud...</h3>
                    ...
                    <div className="text-center space-y-4 max-w-md p-6">
                        <h2 className="text-2xl font-bold text-slate-200">3D Point Cloud Visualizer</h2>
                        <p className="text-slate-400 leading-relaxed">
                            Upload an image to start. The system uses <span className="text-indigo-400 font-medium">Gemini 2.5</span> to infer depth information from a single 2D image and reconstructs it as an interactive 3D particle system.
                        </p>
                    </div>
                )}
             </div>
