> **Nota — backend del proyecto `image_to_xyz`:** este directorio contiene el código de Depth-Anything-V2 con un wrapper FastAPI (`server.py`) que expone el modelo como API REST para el frontend (la raíz de este repo). La sección **"Servidor local (server.py)"** documenta ese wrapper. El resto del README es la documentación oficial del modelo upstream.
